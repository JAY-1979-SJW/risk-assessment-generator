...
SHEET_NAME = "리프트곤돌라사용계획서"
SHEET_HEADING  = "리프트·곤돌라 사용계획서"
SHEET_SUBTITLE = (
    "산업안전보건기준에 관한 규칙 제133조~제148조에 따른 리프트·곤돌라 사용 실무 서식"
    f" ({DOC_ID})"
)
...
def _write_equipment_info(ws, row: int, data: Dict[str, Any]) -> int:
    ...
    row += 1
    _lv(ws, row, "최대 적재하중", v(data, "max_load"),         _L1, _V1S, _V1E)
    _lv(ws, row, "최대 상승 높이", v(data, "max_height"),      _L2, _V2S, _V2E)
    row += 1
    _lv(ws, row, "설치 위치",   v(data, "install_location"),  _L1, _V1S, _V1E)
    _lv(ws, row, "사용 기간",   v(data, "use_period"),        _L2, _V2S, _V2E)
    row += 1
    _lv(ws, row, "운전 자격",   v(data, "operator_license"),  _L1, _V1S, _V1E)
    _lv(ws, row, "운전자 성명", v(data, "operator_name"),     _L2, _V2S, _V2E)
    row += 1
...
def build_lift_gondola_use_plan_excel(
    form_data,
) -> bytes:
    """form_data dict를 받아 리프트·곤돌라 사용계획서 xlsx 바이너리를 반환한다."""
