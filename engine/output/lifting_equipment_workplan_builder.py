"""양중기·호이스트·윈치 작업계획서 builder (EQ-011)."""
...
SHEET_NAME     = "양중기 작업계획서"
SHEET_HEADING  = "양중기·호이스트·윈치 작업계획서"
SHEET_SUBTITLE = (
    "「산업안전보건기준에 관한 규칙」 제132조~제141조 준용"
    f" ({DOC_ID})"
)
...
def _write_equipment_info(ws, row: int, data: Dict[str, Any]) -> int:
    """양중기 장비 핵심 기재사항 (제132조~제141조 준용)."""
    row = _section_header(ws, row, "▶ 장비 정보 (핵심 기재사항)")

    fields = [
        ("장비 종류",     "equipment_type",        "크레인·호이스트·윈치 등"),
        ("장비 모델",     "equipment_model",        ""),
        ("정격하중(t)",   "rated_load",             ""),
        ("최대 인양고(m)", "max_lift_height",        ""),
        ("작업 반경(m)",  "work_radius",            ""),
        ("운전자 자격",   "operator_qualification", ""),
        ("검사 유효기간", "inspection_expiry",      ""),
    ...
    row += 1
    _lv(ws, row, "인양 물체",    v(data, "load_description"),  _L1, _V1S, _V1E)
    _lv(ws, row, "보조장비",     v(data, "auxiliary_equipment"), _L2, _V2S, _V2E)
    return row + 1
...
def build_lifting_equipment_workplan_excel(
    form_data: Mapping[str, Any],
) -> bytes:
    """form_data dict를 받아 양중기·호이스트·윈치 작업계획서 xlsx 바이너리를 반환한다."""
