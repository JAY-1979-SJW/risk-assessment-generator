"""
고소작업대 사용계획서 Excel builder — EQ-005 (v1.0)

법적 근거: 산업안전보건기준에 관한 규칙 제86조 이하
적용 범위: 스카이차·붐리프트·시저리프트 포함
"""
...
SHEET_NAME = "고소작업대사용계획서"
SHEET_HEADING  = "고소작업대 사용계획서"
SHEET_SUBTITLE = (
    "「산업안전보건기준에 관한 규칙」 제86조 이하에 따른 고소작업대 사용계획서"
    f" ({DOC_ID})"
)
...
def _write_meta(ws, row: int, data: Dict[str, Any]) -> int:
    ...
    _lv(ws, row, "작업내용", v(data, "work_summary"),  _L2, _V2S, _V2E)
    row += 1
...
def _write_equipment_info(ws, row: int, data: Dict[str, Any]) -> int:
    ...
    fields = [
        ...
        ("장비 모델명·번호",  "equipment_model",     "제조사·모델명·관리번호"),
        ("최대 작업 높이",    "max_height",          "m 단위"),
        ("최대 작업 하중",    "max_load",            "kg 단위"),
        ("운전자 자격",       "operator_qualification", "해당 자격증 종류·번호"),
        ("작업 반경",         "work_radius",         "장비 선회·이동 반경(m)"),
        ("지반 조건·지지력",  "ground_condition",    "지반 지지력 확인 여부·보강 조치"),
        ("아웃트리거 설치",   "outrigger_plan",      "설치 위치·받침 조치 계획"),
        ("비상조치 방법",     "emergency_measure",   "추락·전도 시 대응 절차·대피 계획"),
    ]
...
def build_aerial_work_platform_use_plan_excel(
    form_data: Mapping[str, Any],
) -> bytes:
    """form_data dict를 받아 고소작업대 사용계획서 xlsx 바이너리를 반환한다."""
