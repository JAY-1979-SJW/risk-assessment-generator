"""
건축물 해체 작업계획서 Excel builder — WP-003 (v1.0)

법적 근거: 산업안전보건기준에 관한 규칙 제38조 제1항 제9호, 제52조~제55조
적용 범위: 해체공사 한정
"""
...
SHEET_NAME = "건축물해체작업계획서"
SHEET_HEADING  = "건축물 해체 작업계획서"
SHEET_SUBTITLE = (
    "「산업안전보건기준에 관한 규칙」 제38조 제1항 제9호·제52조~제55조에 따른 건축물 해체 작업계획서"
    f" ({DOC_ID})"
)
...
def _write_meta(ws, row: int, data: Dict[str, Any]) -> int:
    ...
    _lv(ws, row, "건물 규모", v(data, "building_info"), _L2, _V2S, _V2E)
    row += 1
...
def _write_demolition_plan(ws, row: int, data: Dict[str, Any]) -> int:
    """해체 계획 — 법정 기재사항 (제38조 제1항 제9호, 제52조~제55조)."""
    row = _section_header(ws, row, "▶ 해체 계획 (법정 기재사항)")

    fields = [
        ("해체 공법",           "demolition_method",      "수작업·기계해체·발파 등"),
        ("해체 순서·절차",      "demolition_sequence",    "상층→하층, 외벽→내벽 등"),
        ("가설 구조물 계획",    "temporary_structure",    "가설 방호망·낙하물 방지망 등"),
        ("사전 조사 결과",      "preliminary_survey",     "구조 현황·위험 요소·유해물질 등"),
        ("유해물질 제거 계획",  "hazmat_removal",         "석면·분진 등 사전 처리 계획 [제55조]"),
        ("지하매설물 조치",     "underground_facilities", "가스·전기·통신·상하수도 등 차단·이설"),
        ("인접 시설 보호",      "adjacent_protection",    "인접 건물·도로·지하 구조물 보호 조치"),
        ("작업 구역 설정",      "work_zone",              "출입 통제·방호 구역·대피로"),
        ("비상조치 방법",       "emergency_measure",      "사고 유형별 대응 절차·대피 계획 [제38조]"),
    ]
...
def _write_safety_steps(ws, row: int, data: Dict[str, Any]) -> int:
    ...
    display = min(max(MIN_SAFETY_ROWS, len(items)), MAX_SAFETY_ROWS)
...
def build_building_demolition_workplan_excel(
    form_data: Mapping[str, Any],
) -> bytes:
    """form_data dict를 받아 건축물 해체 작업계획서 xlsx 바이너리를 반환한다."""
    ...
    row = _write_demolition_plan(ws, row, data)
