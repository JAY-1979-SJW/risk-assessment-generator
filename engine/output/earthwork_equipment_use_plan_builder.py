"""덤프·롤러·불도저 사용계획서 builder (EQ-008)."""
...
SHEET_NAME     = "토공장비사용계획서"
SHEET_HEADING  = "덤프·롤러·불도저 사용계획서"
SHEET_SUBTITLE = (
    "「산업안전보건기준에 관한 규칙」 제38조 제1항 제3호, 제170조에 따른 법정 작업계획서"
    f" ({DOC_ID})"
)
...
def _write_equipment_info(ws, row: int, data: Dict[str, Any]) -> int:
    """토공장비 법정 기재사항 (제38조 제1항 제3호, 제170조)."""
    row = _section_header(ws, row, "▶ 장비 정보 (법정 기재사항)")

    pairs = [
        ("장비 종류",       "equipment_type",        "덤프·롤러·불도저 등"),
        ("장비 모델",       "equipment_model",        ""),
        ("장비 용량",       "equipment_capacity",     ""),
...
def build_earthwork_equipment_use_plan_excel(
    form_data: Mapping[str, Any],
) -> bytes:
    """form_data dict를 받아 덤프·롤러·불도저 사용계획서 xlsx 바이너리를 반환한다."""
