...
SHEET_NAME = "임시전기발전기사용계획서"
SHEET_HEADING  = "임시전기·발전기 사용계획서"
SHEET_SUBTITLE = (
    "산업안전보건기준에 관한 규칙 제301조~제330조에 따른 임시전기·발전기 사용 실무 서식"
    f" ({DOC_ID})"
)
...
def _write_equipment_info(ws, row: int, data: Dict[str, Any]) -> int:
    row = _section_header(ws, row, "▶ 장비·설비 정보 (핵심 기재사항)")
    _lv(ws, row, "설비 종류",   v(data, "equipment_type"),    _L1, _V1S, _V1E)
    _lv(ws, row, "장비명/모델", v(data, "equipment_model"),   _L2, _V2S, _V2E)
    row += 1
    _lv(ws, row, "용량(kVA/kW)", v(data, "capacity"),         _L1, _V1S, _V1E)
    _lv(ws, row, "전압(V)",      v(data, "voltage"),          _L2, _V2S, _V2E)
    row += 1
    _lv(ws, row, "사용 기간",   v(data, "use_period"),        _L1, _V1S, _V1E)
    _lv(ws, row, "운전 담당자", v(data, "operator_name"),     _L2, _V2S, _V2E)
    row += 1
    _lv(ws, row, "전기 자격",   v(data, "electrical_license"), _L1, _V1S, _V1E)
    _lv(ws, row, "접지 방법",   v(data, "grounding_method"),  _L2, _V2S, _V2E)
    row += 1
    write_cell(ws, row, 1, 1, "사용 목적",
               font=FONT_BOLD, fill=FILL_LABEL, align=ALIGN_LABEL, height=36)
    write_cell(ws, row, 2, TOTAL_COLS, v(data, "use_purpose"),
               font=FONT_DEFAULT, align=ALIGN_LEFT)
...
def build_temp_power_generator_use_plan_excel(
    form_data,
) -> bytes:
    """form_data dict를 받아 임시전기·발전기 사용계획서 xlsx 바이너리를 반환한다."""
