...
v1.1 changes (correction layer):
- assemble_hazards: filters generic tags, uses text-based classifier as primary source
- assemble_actions: skips single-word generic measures, extracts phrases from raw_text
"""
...
# Verbs that typically close an action clause in Korean safety texts
_ACTION_VERBS = (
    '착용', '설치', '차단', '확인', '점검', '제거', '금지', '비치',
    '배치', '측정', '실시', '교육', '통제', '사용', '금지', '방지',
    '준수', '수행', '관리', '감시', '유지', '고정', '체결',
)

# Minimum character length for a phrase to be kept (excluding trailing verb)
_MIN_PHRASE_LEN = 4
...
def _extract_action_phrases(text: str) -> List[str]:
    """
    Extract action phrases from free text by finding clauses that end with
    an action verb. Minimum 4 chars before the verb.

    Example:
      "안전모 및 안전대 착용 확인, 소화기 비치" -> ["안전모 및 안전대 착용 확인", "소화기 비치"]
    """
    if not text:
        return []

    # Split into clauses at sentence/clause boundaries
    clauses = re.split(r'[,\n·•\-\*\|]', text)
    phrases = []
    for clause in clauses:
        clause = clause.strip()
        if not clause or len(clause) < _MIN_PHRASE_LEN + 2:
            continue
        for verb in _ACTION_VERBS:
            # Check if clause contains the verb (not just starts with it)
            idx = clause.rfind(verb)
            if idx > _MIN_PHRASE_LEN:
                phrase = clause[:idx + len(verb)].strip()
                # Filter: must have at least 4 chars before the verb
                if len(phrase) >= _MIN_PHRASE_LEN + len(verb):
                    # Remove leading punctuation/numbers
                    phrase = re.sub(r'^[\d\.\)\s]+', '', phrase).strip()
                    if phrase and len(phrase) >= _MIN_PHRASE_LEN:
                        phrases.append(phrase)
                        break  # One verb per clause
    return phrases


def _is_generic_action(text: str) -> bool:
    """Return True if action text is a single generic word with no specificity."""
    stripped = text.strip()
    return stripped in _GENERIC_ACTIONS or len(stripped) <= 2


# ── Hazard assembly (corrected) ─────────────────────────────────────────────

def _hazard_frequency_specific(chunks: List[Dict[str, Any]]) -> List[str]:
    """
    Return specific (non-generic) hazard_type values ordered by frequency.
    Generic tags ("위험", "유해", etc.) are excluded.
    """
    freq: Dict[str, int] = {}
    for c in chunks:
...
def assemble_hazards(chunks: List[Dict[str, Any]]) -> List[str]:
    ...
    v1.1: generic tags filtered; text classifier runs on ALL chunks regardless of tag.
    """
    # Step 1: specific tags by frequency (fallback baseline)
    specific_tags = _hazard_frequency_specific(chunks)

    # Step 2: text classifier across all chunks
    ...
    # Classifier results ordered by frequency
    classifier_ordered = [
        k for k, _ in sorted(classifier_freq.items(), key=lambda x: x[1], reverse=True)
    ]

    # Step 3: merge — classifier takes priority, tags supplement
    combined = classifier_ordered + [t for t in specific_tags if t not in classifier_ordered]
    return _dedup_ordered(combined)[:5]


# ── Action assembly (corrected) ─────────────────────────────────────────────

def assemble_actions(chunks: List[Dict[str, Any]]) -> List[str]:
    """
    recommended_actions: phrase-based extraction.

    v1.1 strategy (priority order):
    1. Multi-word control_measure items (>= 5 chars, not generic)
    2. Phrases extracted from raw_text/normalized_text via action-verb patterns
    3. Single-word control_measure items as last resort (filtered)

    Tagged chunks are processed first.
    """
    def _process_chunk(c: dict) -> tuple[List[str], List[str]]:
        good, fallback = [], []
        # control_measure items
    ...
    combined = tagged_good + untagged_good + tagged_fb + untagged_fb
    return _dedup_ordered(combined)[:8]


# ── PPE assembly (unchanged) ────────────────────────────────────────────────

def assemble_ppe(chunks: List[Dict[str, Any]]) -> List[str]:
    """required_ppe: from ppe field, union across all chunks."""
    all_ppe = []
    for c in chunks:
...
def assemble_legal(chunks: List[Dict[str, Any]]) -> List[str]:
    """legal_basis_candidates: from law_ref, deduped."""
    all_refs = []
    for c in chunks:
